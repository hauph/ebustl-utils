...
class TestSTLExtractorInit:
    ...
    def test_mxf_path_required(self):
        ...
        with pytest.raises(ValueError, match="MXF path is required"):
            STLExtractor(mxf_path=None, output_dir="output")
    ...
    def test_mxf_path_must_end_with_mxf(self):
        ...
        with pytest.raises(ValueError, match="MXF path must end with .mxf"):
            STLExtractor(mxf_path="video.mp4", output_dir="output")

    def test_output_dir_required(self):
        """Raise ValueError when output_dir is None."""
        with pytest.raises(ValueError, match="Output directory is required"):
            STLExtractor(mxf_path="video.mxf", output_dir=None)
...
class TestHasTeletextData:
    ...
    def test_returns_true_when_enough_sync_patterns(
        self, mock_extract, mock_video_info, mock_makedirs
    ):
        ...
        extractor = STLExtractor(mxf_path="test.mxf", output_dir="output")
    ...
    @patch.object(STLExtractor, "_get_mxf_video_info")
    @patch.object(STLExtractor, "_extract_raw_payload_from_mxf")
    def test_returns_false_when_few_sync_patterns(
        self, mock_extract, mock_video_info, mock_makedirs
    ):
    ...
    @patch.object(STLExtractor, "_get_mxf_video_info")
    @patch.object(STLExtractor, "_extract_raw_payload_from_mxf")
    def test_returns_false_when_no_sync_patterns(
        self, mock_extract, mock_video_info, mock_makedirs
    ):
...
class TestSTLExtractorExtract:
    ...
    @patch.object(STLExtractor, "_get_mxf_video_info")
    @patch.object(STLExtractor, "_extract_raw_payload_from_mxf")
    def test_raises_when_no_data_extracted(
        self, mock_extract, mock_video_info, mock_makedirs
    ):
    ...
    @patch.object(STLExtractor, "_get_mxf_video_info")
    @patch.object(STLExtractor, "_extract_raw_payload_from_mxf")
    def test_raises_when_no_teletext_data(
        self, mock_extract, mock_video_info, mock_makedirs
    ):
...
class TestGetMxfVideoInfo:
    ...
    @patch.object(STLExtractor, "_extract_raw_payload_from_mxf")
    def test_parses_ffprobe_output(self, mock_extract, mock_run, mock_makedirs):
    ...
    @patch("ebustl_utils.STLExtractor.STLExtractor.subprocess.run")
    @patch.object(STLExtractor, "_extract_raw_payload_from_mxf")
    def test_handles_fractional_frame_rate(self, mock_extract, mock_run, mock_makedirs):
...
class TestExtractRawPayloadFromMxf:
    ...
    @patch.object(STLExtractor, "_get_mxf_video_info")
    def test_detects_data_stream(
        self, mock_video_info, mock_check, mock_run, mock_makedirs
    ):
    ...
    @patch("ebustl_utils.STLExtractor.STLExtractor.subprocess.run")
    @patch.object(STLExtractor, "_get_mxf_video_info")
    def test_raises_when_no_stream_found(
        self, mock_video_info, mock_run, mock_makedirs
    ):
        ...
        with pytest.raises(ValueError, match="No data/subtitle stream found"):
            STLExtractor(mxf_path="test.mxf", output_dir="output")
